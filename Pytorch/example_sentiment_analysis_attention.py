...
class Args:
    def __init__(self):
        ...
        self.n_attention_heads = 5
        self.n_neurons = (200, 100)
...
def extract_vocab_dict_and_msl(sentences_train, sentences_dev, sentences_test):
    """ Tokenizes all the sentences and gets a dictionary id of unique tokens and also the maximum sequence length """
    tokens, ms_len = [], 0
    for sentence in list(sentences_train) + list(sentences_dev) + list(sentences_test):
        tokens_in_sentence = nltk.word_tokenize(sentence)
        if ms_len < len(tokens_in_sentence):
            ms_len = len(tokens_in_sentence)
        tokens += tokens_in_sentence
    token_vocab = {key: i for key, i in zip(set(tokens), range(len(set(tokens))))}
...
class MLPModuleList(nn.Module):
    def __init__(self, vocab_size, embedding_dim, neurons_per_layer):
        super(MLPModuleList, self).__init__()
        # Maps token ids (integers) to a embedding_dim dimensional space
        self.embedding = nn.Embedding(vocab_size+1, embedding_dim, padding_idx=0)
        self.query = nn.Linear(embedding_dim, embedding_dim)
        self.key = nn.Linear(embedding_dim, embedding_dim)
        self.value = nn.Linear(embedding_dim, embedding_dim)
        self.attention = nn.MultiheadAttention(embedding_dim, args.n_attention_heads)
        # MLP part, the input dim to the first layer must be seq_len * embedding_dim
        dims = (args.seq_len*embedding_dim, *neurons_per_layer)
    ...
    def forward(self, x):
        ...
        x, _ = self.attention(q.permute(1, 0, 2), k.permute(1, 0, 2), v.permute(1, 0, 2))
        x = x.permute(1, 0, 2)
...
# %% -------------------------------------- Data Prep ------------------------------------------------------------------
# Loads all the data. We are even going to use the test set to get all the unique words so that we don't encounter
# out-of-vocab words. There are better ways to handle this when we need to use our model to new corpora and we don't
# want to retrain again. Easiest way is using pre-trained word embeddings
...
try:  # Tries to open the vocab dict and the maximum sequence length
    ...
    np.save("exercise_prep_data/max_sequence_length.npy", np.array([msl]))  # Saves msl to numpy
...
# %% -------------------------------------- Training Prep ----------------------------------------------------------
model = MLPModuleList(len(token_ids), args.embedding_dim, args.n_neurons).to(device)
optimizer = torch.optim.Adam(model.parameters(), lr=args.lr)
criterion = nn.CrossEntropyLoss()

# %% -------------------------------------- Training Loop ----------------------------------------------------------
# a = np.copy(model.embedding.weight.data.numpy())
if args.train:
    acc_dev_best = 0
    ...
    for epoch in range(args.n_epochs):
        ...
        loss_train = 0
        model.train()
        for batch in range(len(x_train)//args.batch_size):
        ...
        if acc_dev > acc_dev_best and args.save_model:
            torch.save(model.state_dict(), "mlp_attention_sentiment.pt")
            print("The model has been saved!")
            loss_test_best = loss_test

# %% ------------------------------------------ Final test -------------------------------------------------------------
model.load_state_dict(torch.load("mlp_attention_sentiment.pt"))  # you need to import or define the class again
model.eval()
y_test_pred = acc(x_dev, y_dev, return_labels=True)
print("The accuracy on the test set is", 100*accuracy_score(y_dev.cpu().numpy(), y_test_pred), "%")

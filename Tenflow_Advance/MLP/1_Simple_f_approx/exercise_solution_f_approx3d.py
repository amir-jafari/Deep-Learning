...
class MLP(tf.keras.Model):
    def __init__(self, hidden_dim1, hidden_dim2):
        ...
        self.linear1 = tf.keras.layers.Dense(hidden_dim1, input_shape=(1,))
...
t = f_to_approx(p1, p2)
p = tf.convert_to_tensor(np.hstack((p1.reshape(-1, 1), p2.reshape(-1, 1))))
t = tf.convert_to_tensor(t.reshape(-1, 1))
...
for epoch in range(N_EPOCHS):
    ...
    if epoch % PRINT_LOSS_EVERY == 0:
        print("Epoch {} | Loss {:.10f}".format(epoch, tf.reduce_mean(loss).numpy()))
    # if epoch % VIZ_FIT_EVERY == 0:
        # compare_with_real(p1, p2, t_pred.numpy().reshape(-1, len(p1)), tf.reduce_mean(loss).numpy())

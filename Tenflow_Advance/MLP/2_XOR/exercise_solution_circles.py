...
p = tf.convert_to_tensor(np.vstack((p1, p2)))
t = tf.convert_to_tensor(np.hstack((t1, t2)))
...
# %% -------------------------------------- Check Approx ---------------------------------------------------------------
print(accuracy_score(t.numpy(), tf.argmax(model(p).numpy(), axis=1))*100)
...
p_np = p.numpy()
h = .02
x_min, x_max = p_np[:, 0].min() - 1, p_np[:, 0].max() + 1
y_min, y_max = p_np[:, 1].min() - 1, p_np[:, 1].max() + 1
xx, yy = np.meshgrid(np.arange(x_min, x_max, h),
                     np.arange(y_min, y_max, h))
Z = np.argmax(model(tf.convert_to_tensor(np.c_[xx.ravel(), yy.ravel()])).numpy(), axis=1)
